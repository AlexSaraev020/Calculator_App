...
export const Calculator = () => {
    ...
    const handleCalculate = (symbol: string) => {
    ...
    const buttons = buttonsCreate(rotate)
                ...
                <div className='w-full h-full flex flex-col items-center bg-zinc-900 absolute rounded-2xl z-0'>
                    ...
                    <div className="w-full h-full flex flex-col justify-end items-center">
                        ...
                        <div className="bg-darkGray-500 grid grid-cols-4 grid-rows-5 text-white h-[75%] w-full rounded-2xl place-items-center">
                            {buttons.map(button => (
                                <button key={button.id} onClick={() => handleCalculate(button.symbol)} className={`${button.className} hover:cursor-pointer text-2xl flex items-center justify-center font-semibold rounded-full `} >
                                    {button.icon}
                                </button>
                            ))}
                            <button onClick={() => setRotate(!rotate)} className="bg-blue-500 w-16 h-16 rounded-full text-2xl">
                                🗘
                            </button>
                        </div>
                    </div>
